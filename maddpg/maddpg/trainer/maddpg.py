...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, optimizer_lamda, alpha=None, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        #print('Printing var_rew',var_rew)
        #print('Printing q',q)
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents, t):
        ...
        for i in range(num_sample):
            target_act_next_n = [agents[i].p_debug['target_act'](obs_next_n[i]) for i in range(self.n)]  # WHY IS THIS ON AGENT[0]'s target_act ????????
        ...
        #print("variance_reward", np.var(rew))
